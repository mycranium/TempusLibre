...
function setSetting(field, settingValue) {
  var isStored = (localStorage.hasOwnProperty('settings')) ? true : false; // Check if LocalStorage has 'punches' item.
  if (!isStored) { // If empty, alert no stored punches, freshSetup. punchArray =[]
    //displayAlert(settingsObj, false, "noStoredMsg");
    ...
    if (!workSettings) {
      displayAlert(msgObject["noValidMsg"], false, settingsObj.alertContainers);
      return false;
    } else {
